"""Config flow for the BoxNow parcel tracker integration."""
...
# The tracking-code format was never confirmed against a real parcelId, so:
# accept free text unchanged and validate on nothing but "non-empty".
# Narrowing this to a guessed pattern (length, charset, prefix, checksum)
# risks rejecting a real code outright, which is worse than forwarding a bad
# one and letting the next poll report it as not found.


def normalize_tracking_code(value: str) -> str:
    """Return the tracking code with leading/trailing whitespace stripped.

    No further normalisation — the format is unconfirmed, so this must not
    guess a casing or separator convention the carrier doesn't actually use.
    """
...
def valid_tracking_code(value: str) -> bool:
    ...
    No format is confirmed to validate further against — see the note above.
    """
    return bool(value)
...
class BoxNowConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the BoxNow integration."""
    ...
    ) -> BoxNowOptionsFlowHandler:
        """Return the options flow handler."""
        return BoxNowOptionsFlowHandler()
...
class BoxNowOptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_parcels(
        ...
        No format is confirmed to validate a code against,
        so — unlike most carriers in this suite — there is no "invalid code"
        error here: ``_clean_tracking_codes`` already drops blanks, and
        anything left is accepted as free text.
        """
