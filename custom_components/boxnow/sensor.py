"""Sensor platform for the BoxNow parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: BoxNowConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up BoxNow sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        BoxNowIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            BoxNowParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(BoxNowNextDeliverySensor(coordinator, entry))
    entities.append(BoxNowAwaitingPickupSensor(coordinator, entry))
    entities.append(BoxNowDeliveredParcelsSensor(coordinator, entry))
    entities.append(BoxNowLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class BoxNowIncomingParcelsSensor(
    CoordinatorEntity[BoxNowCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                BoxNowParcelSensor(self.coordinator, self._entry, barcode)
...
class BoxNowParcelSensor(CoordinatorEntity[BoxNowCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked BoxNow parcel."""
    ...
    def __init__(
        self, coordinator: BoxNowCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class BoxNowNextDeliverySensor(
    CoordinatorEntity[BoxNowCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: BoxNowCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class BoxNowAwaitingPickupSensor(
    CoordinatorEntity[BoxNowCoordinator], SensorEntity
):
    """Parcels ready to collect at a BoxNow locker.

    BoxNow is a locker network, so this sensor is wired now per the reference
    carriers (``ha-dhl-nl``, ``ha-dpd``, ``ha-gls``, ``ha-inpost``), even
    though no live sample has confirmed the top-level ``state`` value that
    marks locker-readiness yet. It filters on ``status``
    alone — never on the ``pickup`` bool, which stays hardcoded ``False``
    until the pickup-point field names are confirmed (see
    :func:`custom_components.boxnow.parcels.normalize_parcel`) — so it
    starts reporting real counts the moment ``_STATE_MAP`` gains a confirmed
    entry that maps to ``ParcelStatus.AT_PICKUP_POINT``, with no other code
    change needed.
    """
...
class BoxNowDeliveredParcelsSensor(
    CoordinatorEntity[BoxNowCoordinator], SensorEntity
):
    """Recently delivered tracked BoxNow parcels."""
...
class BoxNowLastUpdateSensor(
    CoordinatorEntity[BoxNowCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when BoxNow was last polled successfully."""
