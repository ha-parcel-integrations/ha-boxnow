"""BoxNow parcel tracker custom component for Home Assistant."""
...
@dataclass
class BoxNowData:
    """Runtime data attached to the BoxNow config entry."""

    client: BoxNowApiClient
    coordinator: BoxNowCoordinator


type BoxNowConfigEntry = ConfigEntry[BoxNowData]
...
async def async_setup_entry(hass: HomeAssistant, entry: BoxNowConfigEntry) -> bool:
    ...
    client = BoxNowApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = BoxNowData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: BoxNowConfigEntry) -> bool:
    """Unload the BoxNow config entry."""
