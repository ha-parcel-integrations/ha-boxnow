"""Coordinator for the BoxNow parcel tracker integration.
...
class BoxNowCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (BoxNowApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, BoxNowApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("BoxNow fetch failed for %s: %s", code, result)
            ...
            if result is None:
                ...
                raws_by_code[code] = self._raw_cache.get(code) or {"parcelId": code}
                continue

            # Tag every response with the code we queried it with — the
            # canonical barcode is the user-supplied parcelId, corroborated
            # (not overridden) by the response's own "id" when present. The
            # response never legitimately carries its own "parcelId" key, so
            # this never overwrites anything real.
            result["parcelId"] = code
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "BoxNow rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("BoxNow unreachable for all tracked parcels")
        ...
        entries = [
            (
                code,
                normalize_parcel(
                    raw, country=self._country, include_history=include_history
                ),
            )
