"""Tests for BoxNow sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.boxnow.const import ParcelStatus
from custom_components.boxnow.sensor import (
    BoxNowAwaitingPickupSensor,
    BoxNowDeliveredParcelsSensor,
    BoxNowIncomingParcelsSensor,
    BoxNowLastUpdateSensor,
    BoxNowNextDeliverySensor,
    BoxNowParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = BoxNowIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = BoxNowParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = BoxNowParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = BoxNowNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = BoxNowNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_awaiting_pickup_counts_only_at_pickup_point_status():
    coordinator = _coordinator([
        _parcel("A", status=ParcelStatus.AT_PICKUP_POINT),
        _parcel("B", status=ParcelStatus.UNKNOWN),
    ])
    sensor = BoxNowAwaitingPickupSensor(coordinator, _entry())
    assert sensor.native_value == 1
    assert sensor.extra_state_attributes["parcels"][0]["barcode"] == "A"


def test_awaiting_pickup_ignores_pickup_bool():
    """Filters on status alone — `pickup` stays hardcoded False for now."""
    coordinator = _coordinator([
        _parcel("A", status=ParcelStatus.AT_PICKUP_POINT, pickup=False),
    ])
    sensor = BoxNowAwaitingPickupSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_delivered_sensor():
    ...
    sensor = BoxNowDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = BoxNowLastUpdateSensor(coordinator, _entry())
