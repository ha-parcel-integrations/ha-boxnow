"""Tests for the BoxNow coordinator: fetching, caching and events.

The parcel mapping itself is covered by ``test_parcels.py``. BoxNow's
top-level ``state`` vocabulary only confirms ``delivered``/``missing`` —
anything else normalizes to ``unknown``, so these tests exercise transitions
between those three (delivered / missing / unknown) rather than the full
``ParcelStatus`` range other carriers can reach.
"""
...
from custom_components.boxnow.api import BoxNowApiError
...
from custom_components.boxnow.coordinator import BoxNowCoordinator
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = BoxNowApiError("HTTP 500")
...
async def test_update_tags_response_with_queried_code(hass):
    """An edge payload without its own id still yields a barcode: the queried code."""
    entry = _entry_with([{CONF_TRACKING_CODE: OTHER_CODE}])
    entry.add_to_hass(hass)
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_sample()
    coordinator = BoxNowCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"parcelId": "GONE"}
...
async def test_event_carries_device_id(hass):
    ...
    client.async_get_parcel.return_value = active_sample()  # unknown
    await coordinator._async_update_data()
    client.async_get_parcel.return_value = missing_sample(ACTIVE_CODE)  # -> problem
    await coordinator._async_update_data()
    await hass.async_block_till_done()
...
async def test_delivery_time_changed_never_fires():
    """No confirmed field supplies an ETA — planned_from/to are always None,

    so this event can never fire yet. Documented as a deliberate limitation
    rather than an oversight."""
    from custom_components.boxnow.parcels import normalize_parcel
