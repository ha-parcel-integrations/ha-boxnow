"""Tests for BoxNow setup and unload."""
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.boxnow.api.BoxNowApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.boxnow_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.boxnow.api.BoxNowApiClient.async_get_parcel",
        new=AsyncMock(side_effect=BoxNowApiError("BoxNow unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    """Swapping which code is tracked spawns/removes the matching per-parcel

    sensor. BoxNow's barcode is always the *queried* code (corroborated
    by, never overridden by, the response's own ``id``), so unlike some
    carriers this cannot be exercised by the API changing its reported id for
    ...
    mock = AsyncMock(side_effect=lambda code: _sample(code))
    with patch("custom_components.boxnow.api.BoxNowApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
        ...
        # Swap which code is tracked: the summary sensor spawns a new
