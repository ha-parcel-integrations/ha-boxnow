...
from custom_components.boxnow.coordinator import (
    BoxNowCoordinator,
...
async def test_update_interval_resumes_once_a_parcel_is_added(hass):
    """Resuming lands on the mid tier here: BoxNow's confirmed `state`

    vocabulary never reaches `out_for_delivery` (the only hot-tier trigger),
    so an active, unmapped-status parcel is always mid, never hot."""
    entry = _entry_with([])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = BoxNowCoordinator(hass, client, entry)
...
async def test_429_raises_update_failed_with_retry_after(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.side_effect = BoxNowApiError(
